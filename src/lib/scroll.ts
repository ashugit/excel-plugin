export class Scroll {
  private vScrollTrack: any;
  private vScrollThumb: any;
  private hScrollTrack: any;
  private hScrollThumb: any;
  ...
  private trackerPixelHeight: any;
  private totalCols: any;
  private trackerPixelWidth: any;
  ...
  setupEventListener() {
    if (this.container.addEventListener) {
      ...
      document
        ...
        .addEventListener("keydown", e => this.doScrollOnArrow(e), false);
    } else {
   ...
   * @param {*} event
   */
  doScrollOnArrow(event: any) {
    switch (event.key) {
      case "ArrowLeft":
        this.doHorizontalScrolling(-1);
        this.reportScrollUpdate(
          Math.abs(this.currentRow),
          Math.abs(this.currentCol)
        );
        break;
      case "ArrowRight":
        this.doHorizontalScrolling(1);
  ...
  doVerticalScrolling(delta) {
    if (delta > 0 && Math.abs(this.currentRow) < this.totalRows - 4) {
      ...
      if (Math.abs(this.currentRow) > this.totalRows - 4)
        this.currentRow = -(this.totalRows - 4);
    ...
    this.vScrollThumb.style.top =
      0 -
      Math.round((this.trackerPixelHeight * this.currentRow) / this.totalRows) +
      "px";
  }
  /**
   *
  ...
  doHorizontalScrolling(delta) {
    if (delta > 0 && Math.abs(this.currentCol) < this.totalCols - 1) {
      ...
      if (Math.abs(this.currentCol) > this.totalCols - 1)
        this.currentCol = -(this.totalCols - 1);
    ...
    this.hScrollThumb.style.left =
      0 -
      Math.round((this.trackerPixelWidth * this.currentCol) / this.totalCols) +
      "px";
  }
